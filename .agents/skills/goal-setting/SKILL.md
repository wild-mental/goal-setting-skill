---
name: goal-setting
description: Designs a complete, paste-ready `/goal` prompt for Claude Code by enforcing the Three Pillars (Feasibility, Demonstrability, Boundedness) and the 4-Element Structure (Goal, Verification, Constraints, Stop Clause). Use when the user mentions `/goal`, "goal-setting", "완료 조건", "장기 실행 에이전트 목표", or wants to turn a vague request into an evaluable long-running goal. Outputs a single-line `/goal ...` string the user can paste into Claude Code. Do not execute `/goal` — this skill only designs the prompt.
---
