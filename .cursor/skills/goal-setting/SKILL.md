---
name: goal-setting
description: Designs a complete, paste-ready `/goal` prompt for Claude Code by enforcing the Three Pillars (Feasibility, Demonstrability, Boundedness) and the 4-Element Structure (Goal, Verification, Constraints, Stop Clause). Use when the user wants to build a `/goal` prompt, mentions `/goal`, "goal-setting", "완료 조건", "장기 실행 에이전트 목표", or whenever a request is too vague to hand to `/goal` directly. Outputs a single-line `/goal ...` string the user can paste into Claude Code.
---
